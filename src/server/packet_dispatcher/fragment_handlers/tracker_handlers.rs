...
impl Server {
    pub(crate) fn subscribe_client<M: Metadata>(&mut self, message: &SubscribeClient<M>) {
        /*
        ...
        match message.client_type {
            ClientType::Audio => {
                let song_metadata_files: Vec<SongMetadata> = message.available_files.iter().map(|file| file.clone()).collect();
                self.database.insert_songs_from_vec(None, &song_metadata_files);
            },
            ClientType::Video => {
